...
impl SolanaAxelarIntegrationMetadata {
    ...
    pub async fn simulate_approve_message(
        ...
        message: MerkleisedMessage,
        verification_session_pda: Pubkey,
    ) -> Result<solana_banks_interface::BanksTransactionResultWithSimulation, solana_program_test::BanksClientError> {
        let ix = self.build_approve_message_instruction(payload_merkle_root, message, verification_session_pda);
