...
#[test_context(ItsTestContext)]
#[tokio::test]
async fn test_deploy_interchain_token_payer_with_data_works(
    ctx: &mut ItsTestContext,
) -> anyhow::Result<()> {
    let salt = solana_sdk::keccak::hash(b"DataOnPayer").0;
    ...
    let mut payer_acc = ctx
        .solana_chain
        .get_account(&payer.pubkey(), &system_program::id())
        .await;
    payer_acc.data = vec![1];
    ctx.solana_chain
        .set_account_state(&ctx.solana_wallet, payer_acc);
